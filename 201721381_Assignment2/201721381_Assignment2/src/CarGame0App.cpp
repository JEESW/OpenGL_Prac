...
float g_camera_rad = 0;
glm::vec3 g_rot(0.f, 0.f, 0.f);
glm::vec3 g_camera_look(0.f, 0.f, 0.f);
...
void Display()
{
	...
	if ( g_camera_mode == 1 )
	{
		...
		projection_matrix = glm::perspective(glm::radians(g_camera_rad), (float)g_window_w / g_window_h, 0.01f, 10000.f);
		glUniformMatrix4fv(m_proj_loc, 1, GL_FALSE, glm::value_ptr(projection_matrix));

		// Camera Transform Matrix 설정.
		view_matrix = glm::lookAt(g_car_poisition + glm::rotateY(g_rot, g_car_rotation_y), g_car_poisition + glm::rotateY(g_camera_look, g_car_rotation_y), glm::vec3(0.f, 1.f, 0.f));
...
void Timer(int value)
{
	...
	glm::vec3 camera_v;
	float radian_v;
	glm::vec3 camera_look_v;

	if (g_camera_mode == 1) {
		camera_v = (glm::vec3(0.1f, 0.7f, -0.1f) - g_rot) / 10.f;
		g_rot += camera_v;
		radian_v = (90.f - g_camera_rad) / 10.f;
		g_camera_rad += radian_v;
		camera_look_v = (glm::vec3(0.1f, 0.f, 2.f) - g_camera_look) / 10.f;
		g_camera_look += camera_look_v;
	}
	else if (g_camera_mode == 2) {
		camera_v = (glm::vec3(0.1f, 2.f, -3.f) - g_rot) / 10.f;
		g_rot += camera_v;
		radian_v = (60.f - g_camera_rad) / 10.f;
		g_camera_rad += radian_v;
		camera_look_v = (glm::vec3(0.1f, -1.f, 6.f) - g_camera_look) / 10.f;
		g_camera_look += camera_look_v;
	}
