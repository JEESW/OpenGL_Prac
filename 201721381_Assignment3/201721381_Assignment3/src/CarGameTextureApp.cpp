...
float g_car_angular_speed = 1.f;	          // 회전 속도 (각속도 - 초당 회전 각)
int shading_mode = 1;
...
void Display()
{
	...
	{
		// 빛의 출발 점과, 진행 방향 설정.
		glm::vec3 light_position(7.f, 2.75f, 0.f);
		glm::vec3 light_dir(-7.f, 0.f, 0.f);
		light_dir = glm::normalize(light_dir);

		light_position = glm::rotateY(light_position, g_car_rotation_y);
		light_dir = glm::rotateY(light_dir, g_car_rotation_y);
		...
		// Spot Light 변수 설정.
		float light_cos_cutoff = cos(glm::radians(40.f));
		glm::vec3 light_indensity(1.0f, 1.0f, 1.0f);
		...
		{
			// 빛의 방향 설정.
			glm::vec3 light_dir2(-1.f, -1.f, 0.f);
			...
			int light_dir_loc2 = glGetUniformLocation(s_program_id, "light_dir2");
	...
	// Moving Car
	{
		...
		glm::mat4 car_T = glm::rotate(g_car_rotation_y, glm::vec3(0.f, 1.f, 0.f));
...
void Timer(int value)
{
	...
	// Turn
	g_car_rotation_y += g_car_angular_speed/60.f;
...
void Keyboard(unsigned char key, int x, int y)
{
	...
	case 'n':
		...
		{
			shading_mode = 2;
			glUniform1i(glGetUniformLocation(s_program_id, "shading_mode"), 2);
		}

		else
		{
			shading_mode = 1;
			glUniform1i(glGetUniformLocation(s_program_id, "shading_mode"), 1);
		}
